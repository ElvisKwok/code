...
/*
 * 使用动态规划加速原理改进最优二叉搜索树算法
 */
void SpeedOptimalBST(double *p, double *q, int n)
{
    int i, j, i1, j1, k, len;
    ...
    for (len = 1; len <= n; len++) {        /* 反对角线方向，当前列入考虑的关键字个数len */
        for (i = 1; i <= n-len+1; i++) {    /* 横坐标i, 给定关键字个数len，对于每一个关键字起始位置i，其总体位置变化范围为i~n-len+1*/
            j = i + len -1;                 /* 纵坐标j，给定对角线和横坐标i，可唯一确定纵坐标j */
            /* 以下两句用于动态规划加速的下标 */
            i1 = root[i][j-1] > i ? root[i][j-1] : i;
            j1 = root[i+1][j] > i ? root[i+1][j] : j;
            e[i][j] = MaxVal;
            w[i][j] = w[i][j-1] + p[j] + q[j];
            for (k = i1; k <= j1; k++) {      /* 求解最小搜索代价e[i][j]，并将根k保存到root[i][j], 细分到每一次求解，关键字的根k的位置上限是j */
                double tmp = e[i][k-1] + e[k+1][j] + w[i][j];
                if (tmp < e[i][j]) {
                    e[i][j] = tmp;
                    root[i][j] = k;
