/* Statement:
   This code is a copy from http://www.cs.dartmouth.edu/~cs57/Project/bellman-ford.c57
   I use it just for personal study.
   Elvis Kwok
 */
...
int main(void) {
    ...
    /* vertex i's first neighor vertex's arc start from index first[i] 
       vertex No.1 has arc No.1, No.2.
       vertex No.2 has arc No.3, No.4, No.5.
       ...
       vertex No.i has arc No.first[i], ..., No.(first[i+1]-1)
     */
    first[1] = 1;
    first[2] = 3;
    first[3] = 6;
    ...
    /* node[i] is arc i's head(tail->head)
       arc No.1, (1, 2)
       arc No.2, (1, 4)
       ...
       arc No.i, (i, node[i])
    ...
    /* the arc's index of next arc of arc i.
       next[3] = 4 means after excution of arc No.3, next arc to be excuted is No.4.
       next[2] = 0 means arc No.2 is the last arc for current iteration of relax.
    */
    next[1] = 2;
    next[2] = 0;
    next[3] = 4;
