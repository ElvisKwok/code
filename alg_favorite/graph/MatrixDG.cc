...
public:
	MatrixDG() { }
	MatrixDG(string vexs[], int vlen, string edges[][2], int elen);
	~MatrixDG();
 ...
 * 构造函数
 */
MatrixDG::MatrixDG(string vexs[], int vlen, string edges[][2], int elen) {
	// 初始化顶点数，边数
	mVexNum = vlen;
	mEdgNum = elen;
...
int main()
{
    ...
    //int elen = sizeof(edges)/sizeof(edges[0]);
   	...
   	MatrixDG* pG = new MatrixDG(vexs, vlen, edges, elen);
	pG->print();
	pG->DFS();
	pG->BFS();
